...
void fat32_mbr_check(struct mbr *mbr) {
    int any_valid_entries = 0;
    for (int i = 0; i < 4; i++) {
        uint8_t part_type = mbr->part_tab[i].part_type;
        any_valid_entries = any_valid_entries || part_type == 0x0b || part_type == 0x0c;
    }
    assert(any_valid_entries);
