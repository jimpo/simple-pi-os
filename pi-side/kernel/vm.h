...
#define SDRAM_SIZE 512 // MiB
#define N_VIRT_PAGES (1 << 12)
#define N_PHYS_PAGES SDRAM_SIZE
...
typedef fld_t trans_table_t[N_VIRT_PAGES];

// base is the physical address of the array of page translation tables.
// We reserve enough space for one translation table per ASID.
extern trans_table_t* const TT_BASE;

typedef struct {
    ASID asid;

    trans_table_t* vaddr_map;
    ...
    struct inv_entry {
        uint16_t
                ...
                sbz:3,
                vaddr:12;
    } paddr_map[N_PHYS_PAGES];
} page_table_t;

// This maintains a global mapping of physical memory pages to ASIDs (process
// IDs as identified to the MMU). This is to avoid collisions on physical pages
...
// Bottom 8 bits of the argument are the ASID, top 24 are 0.
// See ARM reference B4-46. See ARM reference B4-46.
void mmu_inv_asid(unsigned asid);
