...
void sd_preinit() {
	...
	static volatile unsigned* GPHEN1 = (volatile unsigned*) 0x20200068;

	// GPIO_CD
	gpio_set_function(GPIO_PIN47, GPIO_FUNC_INPUT);
	gpio_set_pud(GPIO_PIN47, GPIO_PUD_PULLUP);

	unsigned r;
	r=*GPHEN1; r|=1<<15; *GPHEN1=r;

    // GPIO_CLK, GPIO_CMD, GPIO_DAT0, GPIO_DAT1, GPIO_DAT2, GPIO_DAT3
...
{--------------------------------------------------------------------------*/
#define waitMicro delay_us							// Waits a number of microseconds
#define TICKCOUNT timer_get_time					// Gets current system clock value in microseconds.
...
static void unpack_csd(struct CSD* csd)
{
	...
	*p = *EMMC_RESP3;
	...
	if (csd->csd_structure == 0x1) {
		LOG_DEBUG("CSD 2.0: ver2_c_size = %d\t  card capacity: %u\n",
			csd->ver2_c_size, sdCard.CardCapacity);
	}
	else {
		LOG_DEBUG("CSD 1.0: c_size = %d\t  c_size_mult=%d\t card capacity: %u\n"
...
static bool LoadDrivePartition (printhandler prn_basic) {
	...
	if (prn_basic) prn_basic("First Sector: %u, Data Sectors: %u, TotalClusters: %u, RootCluster: %u\n",
